...
class SWECLongEEGData(EEGDataset):
    _SAMPLING_RATE = 512
    ...
    def setup(self, stage: str) -> None:
        if stage == "fit":
            ...
            self.dataset_train = SWECLongEEGDataset(
                folder=self.folder,
            ...
            if self.val_patients:
                ...
                self.dataset_val = SWECLongEEGDataset(
                    folder=self.folder,
        ...
        if stage == "test" or stage == "predict":
            ...
            for patient in self.test_patients:
                dataset_test = SWECLongEEGDataset(
                    folder=self.folder,
                    n_patient=[patient],
                    window_n=self.segment_n,
                    window=self.window_size,
...
class SWECLongEEGDataset(Dataset[EEGBatch]):
    _URL = "http://ieeg-swez.ethz.ch/super-long-term_dataset/"
    _SAMPLING_RATE = 512
    ...
    def map_items(self) -> None:
        ...
        for i in range(len(patients_slowdown_indices)):
            ...
            for seiz in patient_seizures:
                ...
                #     seiz_before_end = 0
                # else:
                #     seiz_before_end = patients_slowdown_indices[i][(seiz - 1) * 2 - 1]
                # if seiz == n_seizures:
                #     seiz_after_begin = self.patients_length[i]
                # else:
                #     seiz_after_begin = patients_slowdown_indices[i][(seiz) * 2]
                # seiz_begin_idx = patients_slowdown_indices[i][(seiz - 1) * 2]
                # take_begin_idx = max(
                #     seiz_begin_idx - 30 * 60 // self.stride,
                #     seiz_begin_idx - (seiz_begin_idx - seiz_before_end) // 2,
                # )
                # seiz_end_idx = patients_slowdown_indices[i][(seiz - 1) * 2 + 1]
                # take_end_idx = min(
                #     seiz_end_idx + 30 * 60 // self.stride - self.window // self.stride,
                #     seiz_end_idx + (seiz_after_begin - seiz_end_idx) // 2,
                # )
                # beg = max(int(patient_begin + take_begin_idx), self.start_idx)
                # end = max(int(patient_begin + take_end_idx), self.start_idx)
                if self.balanced:
                    inside_seizure = False
                    for s in self.patients_seizures_indices[i]:
                        s += patient_begin
    ...
    def get_trainable_delta(self) -> None:
        ...
        for i, _ in enumerate(self.patient_files):
            ...
            for bound in zip(bin_edges, bin_edges[1:]):
                ...
                #     patient_begin
                #     + self._find_index_from_second(
                #         bin_bound_sec[selected_length * 2 + 1],
                #         self.patients_slowdown_indices[i],
                #     ),
                # )

                # trainable_to_add = list(
                #     set(list(bin_trainable_indices))
                #     & set(list(range(len(self.patients))))
                # )
